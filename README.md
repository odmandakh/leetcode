...
All three are **required** for the Daily Loop below, but none of them live in this repo — they're maintained in a separate personal skills repo (`personal/skills`) and installed globally by that repo's `scripts/install.sh`, which symlinks each `skills/<name>/` folder into `~/.claude/skills/<name>`. Because the install is global, the same `cpp-pro`, `leetcode-teacher`, and `leetcode-import` skills are available across every repo on the machine, not just this one. Cloning this repo alone is **not** sufficient to get them — you also need to clone `personal/skills` and run its install script. `.agents/skills/` is Zed's *native*-agent skill format — unused by this repo's actual workflow, so it isn't tracked at all; don't recreate it unless you're switching off Claude Code. `.zed/` (Zed debugger/task/editor config) is gitignored as personal, machine-specific setup, not repo content.
