...
class Solution {
 public:
  int maxWalls(vector<int>& robots, vector<int>& distance, vector<int>& walls) {
    ...
    auto solve = [&](auto&& solve, int robotIdx, int prevDirection) -> int {
      ...
      maxDestroyed = solve(solve, robotIdx - 1, 0) + wallsDestroyedLeft;
      ...
      if (robotIdx + 1 < n) {
        if (prevDirection == 0) {
          ...
          rightBound =
              min(rightBound, robotInfo[robotIdx + 1].first - robotInfo[robotIdx + 1].second - 1);
        } else {
      ...
      maxDestroyed = max(maxDestroyed, solve(solve, robotIdx - 1, 1) + wallsDestroyedRight);
